...
pushd $ODK_SRC_TMP > /dev/null 2>&1
# Decompress the ODK war file
jar -xvf $BASE_DIR/ODKAggregate.war > /dev/null 2>&1

pushd $ODK_SETTINGS_TMP > /dev/null 2>&1
...
rm -f  *.bak
jar cf $ODK_BACKUP/ODKAggregate-settings.jar ./*
popd > /dev/null 2>&1

# mv -f $ODK_SRC_TMP/WEB-INF/lib/ODKAggregate-settings.jar $ODK_BACKUP/ODKAggregate-settings.jar
mv -f $ODK_BACKUP/ODKAggregate-settings.jar $ODK_SRC_TMP/WEB-INF/lib/ODKAggregate-settings.jar
# cp -f $BASE_DIR/ODKAggregate.war $BASE_DIR/ODKAggregate-$(date +%s).bak
